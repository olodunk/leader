...
# Since I don't have a live session here to call the API via HTTP easily (CSRF/Auth),
# I will just run the calculate logic via a standalone script or tell the user to click it.
# Actually, I can just use the DB connection to trigger the calculation logic.
...
def trigger_calculate():
    ...
    # Logic copied from team_score_details_calculate in app.py
    sql = '''
        SELECT 
            d.dept_name, 
    ...
    conn.commit()
    print(f"Recalculated {len(data_to_insert)} records.")
    conn.close()
